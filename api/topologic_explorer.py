...
def _safe_path(*components):
    """Join path components under APP_PATH, rejecting traversal attempts."""
    path = os.path.realpath(os.path.join(APP_PATH, *components))
...
def get_config(table, full_config: bool = False):
    if full_config is True:
        ...
        config["topics_words"] = read_json_config(_safe_path(table, "topic_words.json"))
        config["appConfig"] = read_json_config(_safe_path(table, "appConfig.json"))
        return config
