...
def save_word_distribution(distribution, file_path):
    with open(file_path, "w") as output:
        words, weights = zip(*distribution)
        json.dump({"labels": words, "data": weights}, output)
...
def save_topic_distribution(distribution, file_path):
    with open(file_path, "w") as output:
        ...
        json.dump({"labels": topics, "data": weights}, output)


def save_topic_evolution(evolution, file_path):
    with open(file_path, "w") as output:
        dates, frequencies = zip(*evolution)
        json.dump({"labels": dates, "data": frequencies}, output)
...
def save_topic_cloud(topic_model, file_path):
    ...
    for i in range(topic_model.nb_topics):
        description = []
        for weighted_word in topic_model.top_words(i, 3):
            description.append(weighted_word[0])
        json_nodes.append(
            {"name": i, "frequency": topic_model.topic_frequency(i), "description": ", ".join(description), "group": i}
        )
